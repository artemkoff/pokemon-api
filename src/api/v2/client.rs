...
impl ApiClient {
    ...
    pub(crate) async fn request_api<T: Into<String>>(&self, req: T) -> Result<Response> {
        self.client.get(format!("{}/{}", self.url, req.into()).as_str())
            .send()
            .await
    ...
    where P: Sized + serde::de::DeserializeOwned {
        self.client.get(format!("{}/{}", self.url, req.into()).as_str())
            .send()
            .await
            .map_err(|err| crate::Error::from(err))?
    ...
    pub(crate) async fn request_api_object_paginated<P, T: Into<String>>(&self, req: T, offset: usize, limit: usize) -> Result<P>
    where P: Sized + serde::de::DeserializeOwned {
        self.client.get(format!("{}/{}?offset={}&limit={}", self.url, req.into(), offset, limit).as_str())
            .send()
            .await
            .map_err(|err| crate::Error::from(err))?
            .json::<P>()
